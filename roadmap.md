...
- 2026-06-01: `Entra ID` REST dispatch completed. Notes: the v2 user/group controllers now dispatch to the Graph provider for Entra-backed domains (list/get/search/exists/create/update/delete + membership list/add/remove/replace) via `BaseController.ResolveProvider`/`IsEntraDomain`/`RunWithProviderAsync`, with `DirectoryErrorMapper` error mapping and audit scope; LDAP path unchanged; `GroupCreateRequest.GroupType` added and `DN` relaxed to per-backend validation; 7 controller-dispatch unit tests added (full suite 554 passing). This closes the controller-wiring item deferred through Stages 6–10.
